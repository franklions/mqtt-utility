...
public class ConnOpts extends JPanel implements ActionListener {
	...
	public ConnOpts( MQTTFrame mgr, Properties props ) {
        ...
        // Create a username password field
        userName = new JTextField( "username", 15 );
        userName.setMaximumSize( MQTTFrame.TEXT_FIELD_DIMENSION );

        password = new JTextField( "password", 15 );
        password.setMaximumSize( MQTTFrame.TEXT_FIELD_DIMENSION );

        JPanel user = new JPanel();
        user.setLayout( new FlowLayout( FlowLayout.LEFT) );

        user.add( new JLabel("UserName:", SwingConstants.LEFT) );
        user.add(userName);
